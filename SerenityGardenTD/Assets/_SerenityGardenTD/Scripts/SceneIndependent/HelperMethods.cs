...
namespace SerenityGarden
{
    public static class HelperMethods
    {
        ...
        /// <param name="objBounds"></param>
        /// <returns></returns>
        public static bool FindBoundsSkinnedMesh(GameObject target, out Bounds objBounds)
        {
            objBounds = new Bounds();
            SkinnedMeshRenderer targetMeshRenderer = target.GetComponent<SkinnedMeshRenderer>();
            ...
            {
                //Otherwise, check if it has children
                List<SkinnedMeshRenderer> childObj = new List<SkinnedMeshRenderer>();
                AddAllChildrenRecursively(childObj, target.transform);
                ...
                //If it does have children, then calculate the min and max bounds
                foreach (SkinnedMeshRenderer rend in childObj)
                {
                    if (boundsMin.x > rend.bounds.min.x)
                        boundsMin.x = rend.bounds.min.x;
                    if (boundsMin.y > rend.bounds.min.y)
        ...
        /// <param name="childObj"></param>
        /// <param name="parent"></param>
        private static void AddAllChildrenRecursively(List<SkinnedMeshRenderer> childObj, Transform parent)
        {
            foreach (Transform child in parent)
            {
